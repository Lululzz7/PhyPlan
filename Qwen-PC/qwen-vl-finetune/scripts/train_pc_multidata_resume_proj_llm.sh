...
# ===== 基础路径（可覆盖） =====
# 根目录（整模型分片 + 配置所在处）
MODEL_DIR=${MODEL_DIR:-"/e2e-data/evad-tech-vla/luzheng/Qwen3-VL-main/outputs/pc_multidata_resume_proj_llm_full_8gpu_cont9/checkpoint-1500"}
# 继续训练的输出目录
OUT=${OUT:-"${ROOT_DIR}/outputs/resume_proj_llm"}
# 训练数据（已合并）
DATA=${DATA:-"${DEMO_DIR}/pointcloud_qa_combined.json"}

# 点云 projector/backbone 初始化
POINT_PROJ_CKPT=${POINT_PROJ_CKPT:-"/e2e-data/evad-tech-vla/luzheng/Qwen3-VL-main/outputs/pc_multidata_resume_proj_llm_full_8gpu_cont9/point_proj_step-1500.bin"}
