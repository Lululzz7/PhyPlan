    ...
    def forward(
        ...
        **kwargs,
        ...
        if point_cloud_embeds is not None:
            ...
            # 仅平移“时间/序列”维度（Row 0），保持 H/W 网格坐标不变；
            # 点云段的 H/W 设为 0，时间维从 0..P-1。
            pos = position_ids.clone()
            # 先对时间/序列维统一平移 P；
            pos[0] = pos[0] + pc_seq_len
            # 为严格对齐原生 Qwen3‑VL 的文本位 mRoPE 结构（文本 token 满足 T=H=W），
            # 对文本位（非视觉位）再将 H/W 维也加上 P；视觉位(H/W 网格)保持不变。
            if visual_pos_masks is not None:
                # visual_pos_masks: (B, L) -> 文本位置 mask = ~visual_pos_masks
                text_mask = (~visual_pos_masks).to(pos.dtype).unsqueeze(0)  # (1, B, L)
                pos[1:] = pos[1:] + text_mask * pc_seq_len
            else:
                # 无视觉位时，全为文本位：三维保持一致性
                pos[1:] = pos[1:] + pc_seq_len
            # 将点云段也按“文本风格”设定 mRoPE：T=H=W=0..P-1，更符合 LLM 对非视觉 token 的位置约定
            ...
            # 重新计算 rope_deltas：基于已修正的 position_ids 与“有效长度”按公式
            # rope_deltas = max_position_ids + 1 - length 计算，避免依赖网格信息。
            # 1) 计算每样本有效长度 length: 优先使用 varlen 的 cu_seqlens，其次 4D full_attention 主对角，
            #    再次 2D mask；若均不可得，回退为当前序列长度 S。
            S = position_ids.shape[-1]
            lengths = None
            if isinstance(attention_mask, dict):
                cu = attention_mask.get("cu_seqlens_q", None) or attention_mask.get("cu_seqlens", None)
                if isinstance(cu, torch.Tensor) and cu.ndim == 1 and cu.numel() >= 2:
                    lens = (cu[1:] - cu[:-1]).to(dtype=position_ids.dtype, device=position_ids.device)
                    lengths = lens.view(-1, 1)
                else:
                    fa = attention_mask.get("full_attention", None)
                    if isinstance(fa, torch.Tensor):
                        if fa.ndim == 4:
                            diag = torch.diagonal(fa[:, 0], dim1=1, dim2=2)
                            if diag.dtype.is_floating_point:
                                diag = diag / torch.finfo(diag.dtype).min
                                diag = (1.0 - diag).int()
                            lengths = diag.sum(dim=-1, keepdim=True).to(dtype=position_ids.dtype, device=position_ids.device)
                        elif fa.ndim == 2:
                            # 2D 情况：0/1 可见位
                            if fa.dtype == torch.bool:
                                vis2d = fa.to(torch.int)
                                ...
                                vis2d = fa.to(torch.int)
                            lengths = vis2d.sum(dim=-1, keepdim=True).to(dtype=position_ids.dtype, device=position_ids.device)
            elif isinstance(attention_mask, torch.Tensor) and attention_mask.ndim == 2:
                lengths = attention_mask.to(dtype=position_ids.dtype, device=position_ids.device).sum(dim=-1, keepdim=True)
            if lengths is None:
                B = position_ids.shape[1]
                lengths = torch.full((B, 1), S, dtype=position_ids.dtype, device=position_ids.device)

            # 2) 计算每样本最大位置索引（跨三行与序列维）
            max_pos = position_ids.max(dim=0, keepdim=False)[0].max(dim=-1, keepdim=True)[0]
            self.rope_deltas = (max_pos + 1 - lengths)
