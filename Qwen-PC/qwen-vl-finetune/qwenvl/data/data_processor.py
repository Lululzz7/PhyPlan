...
def preprocess_qwen_visual(
    ...
    # 使用增量方式标注 assistant 片段，避免依赖硬编码特殊 Token ID
    prev_len = 0
    for t_idx in range(len(messages)):
        partial = processor.apply_chat_template(
            messages[: t_idx + 1], tokenize=True, return_dict=True, return_tensors="pt"
        )
        ids_t = partial["input_ids"]
        if isinstance(ids_t, list):
            ids_t = torch.tensor(ids_t).unsqueeze(0)
        cur_len = ids_t[0].size(0)
        if messages[t_idx]["role"] == "assistant":
            labels[0, prev_len:cur_len] = input_ids[0, prev_len:cur_len]
        prev_len = cur_len
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, processor, data_args):
        ...
        self.video_max_total_pixels = getattr(
            data_args, "video_max_total_pixels", 1664 * 32 * 32
        )
        self.video_min_total_pixels = getattr(
            data_args, "video_min_total_pixels", 256 * 32 * 32
        )
