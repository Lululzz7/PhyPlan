...
# ===== 基本参数（可通过环境变量覆盖） =====
MODEL=${MODEL:-"/e2e-data/evad-tech-vla/luzheng/Qwen3-VL-main/outputs/pc_multidata_resume_proj_llm_full_8gpu_cont9/checkpoint-1000"}                              # 预训练模型或本地路径
TRAIN_FILE=${TRAIN_FILE:-"${ROOT_DIR}/qwen-vl-finetune/demo/AD_train_2D.jsonl"}
OUT=${OUT:-"${ROOT_DIR}/outputs/run_ad2d_llm_zero2_pc"}                  # 输出目录

EPOCHS=${EPOCHS:-1}
BATCH_PER_DEVICE=${BATCH_PER_DEVICE:-8}
GRAD_ACCUM=${GRAD_ACCUM:-32}
LR=${LR:-1e-5}
BF16=${BF16:-1}    # 1: 启用 bf16；0: 关闭
FP16=${FP16:-0}    # 1: 启用 fp16；0: 关闭（与 bf16 二选一）
