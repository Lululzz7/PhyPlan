...
# 单机八卡训练脚本（每卡 batch size = 1）
# 训练数据来源：ScanQA、Scene Caption、Scene-30K、SQA3D 合并。
# Epoch = 3。

ROOT_DIR=$(cd "$(dirname "$0")"/../.. && pwd)
DEMO_DIR="${ROOT_DIR}/qwen-vl-finetune/demo"

# 四份训练数据（默认指向仓库内 demo 路径，可通过环境变量覆盖）
...
OUT=${OUT:-"${ROOT_DIR}/outputs/pc_multidata_from_scratch"}
SAVE_PROJ=${SAVE_PROJ:-"${OUT}/point_proj.bin"}

# 点云骨干（如需指定预训练权重请设置环境变量 POINT_BACKBONE_CKPT）
...
# 单机八卡启动（需确保 torchrun 可用）
# Print training plan and save targets
