...
# ===== 基本参数（可通过环境变量覆盖） =====
MODEL=${MODEL:-"/e2e-data/evad-tech-vla/luzheng/Qwen3-VL-main/outputs/pc_multidata_resume_proj_llm_full_8gpu_cont9/checkpoint-1500"}                              # 预训练模型或本地路径
TRAIN_FILE=${TRAIN_FILE:-"${ROOT_DIR}/qwen-vl-finetune/demo/AD_train_2D.jsonl"}
OUT=${OUT:-"${ROOT_DIR}/outputs/run_2d_llm"}                  # 输出目录
