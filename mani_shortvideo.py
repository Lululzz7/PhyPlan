...
class ScriptConfig:
    ...
    OUTPUT_BASE_FOLDER: str = "causal_spafa_plan_dataset_short"
...
PLANNING_CONFIG = ScriptConfig(
    VIDEO_PATH="8.mp4",
    OUTPUT_BASE_FOLDER="causal_spafa_plan_dataset_short",
...
Your mission is to produce a single-step plan with fine-grained causal, spatial, and affordance annotations for a short video. This plan must explain not just *what* happened, but precisely *how* and *why* it is happening from a physical standpoint by inferring the dynamics implied within each key moment.
...
def create_planning_user_prompt(num_frames: int, image_dimensions: Tuple[int, int]) -> str:
    ...
    return f"""
You are a world-class AI, a doctorate-level expert in physics, robotics, and cognitive science, acting as a **Physical Interaction Analyst and Causal Analyst**. Your primary mission is to deconstruct observed human actions from video frames into their most fundamental causal, kinetic, and physical principles. You must think step-by-step with extreme precision, logical rigor, and unwavering adherence to the specified JSON schema. Your output is not just a description; it is a scientific annotation.

Analyze the provided {num_frames} frames, which are uniformly sampled from a continuous short video of a task. Your task is to reverse-engineer the high-level goal and generate a deeply detailed annotation. This annotation has only one  fine-grained, logical step.
      ...
      "critical_frames": [
        {{
          ...
          "spatial_preconditions": [
            {{ "relation": "A visually verifiable spatial relationship that is true *at the moment of this frame*. Be comprehensive. e.g., 'knife is in contact with and perpendicular to the cucumber'", "objects": ["knife", "cucumber"], "truth": true }}
          ],
...
3.  **Focus on the Critical Frame:** `spatial_preconditions` and `affordance_preconditions` MUST comprehensively describe the state of the world **in that specific frame**.
4.  **Principle of Pivotal Moments & Multiple Frames:** One `step` can and often should contain multiple `critical_frames`. Identify ALL distinct and meaningful pivotal moments. Exactly ONE step (step_id=1). No additional steps.
...
9.  **JSON Structure is Paramount:** Adhere strictly to the schema. Note that `causal_chain` and `affordance_hotspot` are **SIBLING** objects; `affordance_hotspot` is **NOT** inside `causal_chain`. It must be noted that "causal_chain" does not include "affordance_hotspot".
10. **High requirements for single step planning:** The one step plan must be detailed and comprehensive, representing all events implied by the frames. The step must be specific and detailed.
11. **One step planning is a Must:** Using these image frames extracted from the video, generate a single-step planning process. Avoid dividing it into multiple sub-steps; instead, select only a few critical frames from a single step to provide a detailed description of that step.
12. **Principle of Visual Anchorability:** While you are not selecting frames now, every critical_frame you describe must correspond to a visually distinct and unambiguous moment. Your descriptions should be "anchor-able" to a plausible visual snapshot. Do not describe events that are inherently invisible or highly ambiguous from a third-person perspective.
---
...
def main():
    ...
    input_dir = os.environ.get("INPUT_VIDEO_DIRECTORY", "/e2e-data/evad-tech-vla/luzheng/ICML/short")
