...
- "image_patch_size": `14` for Qwen2.5-VL and `16` for Qwen3-VL. Default set to `16` for Qwen3-VL guidance.
...
```python
# For legacy Qwen2.5-VL you can simply call 
...
📌 Point Cloud Modality (Qwen3-VL + PointLLM-style)
- We support point clouds as an additional modality. The preprocessing strictly follows PointLLM: center to origin, normalize to unit sphere, and optionally farthest point sampling to a fixed point count.
- For colored point clouds with RGB channels (N,6), we normalize colors to [0,1]. If values > 1 are detected (e.g., [0,255] uint8), we scale by 255 and clip to [0,1].
- Injection policy differs from classic PointLLM: instead of textual "<point>" placeholders, we prepend point tokens at the beginning of the sequence, and adjust attention_mask/position_ids/labels accordingly. Hence, your prompts do NOT need to contain "<point>" explicitly.
- Ensure the point backbone config (e.g., PointBERT YAML + checkpoints) matches between training and inference.
