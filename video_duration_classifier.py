...
This script scans videos inside a provided source directory.
It checks if the video already exists in the output directories to support resume.
"""
...
class ScriptConfig:
    ...
    source_folder: str = "/e2e-data/embodied-research-data/HumanData/Ego4D/ego4d_data/v2/full_scale"
...
def collect_video_paths(config: ScriptConfig) -> Iterable[str]:
    ...
    entries = sorted(os.listdir(config.source_folder))
    for name in entries:
        candidate = os.path.join(config.source_folder, name)
...
def process_video(
    ...
    # 处理正常分类
    needs_trim = duration > config.max_duration_seconds
    effective_duration = min(duration, config.max_duration_seconds)
    
    # 注意：如果被剪辑了，它依然属于 long (因为剪辑后是3分钟)，这里逻辑复用 categorize
    category = categorize_duration(effective_duration, config)
    output_path = os.path.join(destinations[category], filename)

    print(f"[INFO] 处理中 ({category}): {filename} [{duration:.2f}s]")

    if needs_trim:
        processed_duration = trim_video(
            video_path, output_path, fps, config.max_duration_seconds, config
        )
        if processed_duration <= 0:
            if os.path.exists(output_path):
                os.remove(output_path)
            print(f"[ERROR] 剪辑写入失败: {filename}")
            return "error", 0.0
        return category, processed_duration
