...
# -*- coding: utf-8 -*-
"""
TypeA 专用生成脚本（复用 generate_phyplan_api.py 的所有逻辑与 Prompt）

功能：
- 仅处理 TypeA 根目录（按自然命名顺序），逐项从前到后生成。
- 实时保存 data.jsonl，并实时更新合并 data.json（count + A/B 两段，TypeA 写入 A 段）。
- 其他逻辑（任务、Prompt、字段、QA、图片配对、不传图入模、断点重续）完全沿用 generate_phyplan_api.py。
"""
...
def _load_gpapi():
    ...
    mod_path = os.path.join(here, 'generate_phyplan_api.py')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='TypeA-only generator (QA, fields-only, live combined save).')
    parser.add_argument('--typeA-root', default='/e2e-data/evad-tech-vla/luzheng/ICML/causal_spafa_plan_dataset_long_kitchen')
    parser.add_argument('--output-dir', default='/e2e-data/evad-tech-vla/luzheng/ICML/phyplan_output_api')
